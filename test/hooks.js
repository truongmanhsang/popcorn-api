...
hooks.beforeAll((t, done) => {
  ...
  const animemovie = Util.Instance
    .importCollection('anime', './test/movie.json')
...
hooks.before('Logs > Error Logs > Get Error Logs', t => {
  t.skip = true
})

hooks.before('Export > Export > Export Collection', t => {
  t.skip = true
})
